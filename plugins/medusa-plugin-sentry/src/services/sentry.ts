...
export default class SentryService extends TransactionBaseService {
	...
	protected async fetchSentryData({
		...
		const {
			data: { data, meta },
			headers,
		} = await this.fetchSentry<{ data: { data: SentryFetchResult['data']; meta: unknown }; headers: any }>({
			organisation,
			token,
			queryParams,
		});

		const nextCursor: any = SentryService.buildNextCursor(headers['link']);
		const currentCursor = SentryService.buildCurrentCursor(headers['link'], perPage ? perPage : 0);

		let prevCursor: any;
		if (currentCursor) {
			prevCursor = SentryService.buildPrevCursor(currentCursor, perPage ? perPage : 0);
		}

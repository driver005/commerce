...
export default async ({
    ...
    await Promise.all(
        resolved.map(async (pluginDetails) =>
            runLoaders(pluginDetails, container)
        )
    )
...
export function registerStrategies(
    ...
    const files = glob.sync(`./strategies/[!__]*.js`, {
                ...
                if (!('taxCalculationStrategy' in registeredServices)) {
                    container.register({
                        taxCalculationStrategy: asFunction(
                            (cradle) =>
                                new module(cradle, pluginDetails.options)
                        ).singleton(),
                    })
...
function registerMedusaMiddleware(
    ...
    container.register({ middlewareService: asClass(MiddlewareService) })
    
    let module
    try {
        module =
            require(`${pluginDetails.resolve}/api/medusa-middleware`).default
    } catch (err) {
        return
    }
...
function registerCoreRouters(
    ...
    const middlewareService =
        container.resolve<MiddlewareService>('middlewareService', { allowUnregistered: true })
    
    const adminFiles = glob.sync(`./api/admin/[!__]*.js`,  { cwd: pluginDetails.resolve })
    const storeFiles = glob.sync(`./api/store/[!__]*.js`,  { cwd: pluginDetails.resolve })
...
function registerApi(
    ...
    try {
        ...
        if (
            err.message !== `Cannot find module '${pluginDetails.resolve}/api'`
        ) {
            logger.progress(
                activityId,
...
export async function registerServices(
    ...
    const files = glob.sync(`./services/[!__]*.js`, { cwd: pluginDetails.resolve })
        ...
        if (existsSync(resolvedPath)) {
            if (existsSync(`${resolvedPath}/package.json`)) {
                ...
                return {
                    ...
                    version:
                        packageJSON.version ||
                        createFileContentHash(resolvedPath, `**`),
                }
            } else {
                // Make package.json a requirement for local plugins too
                throw new Error(
                    `Plugin ${pluginName} requires a package.json file`
                )
            }
        }
    }

...
describe('plugins loader', () => {
    ...
    container.registerAdd('logger', asValue(Logger))
    ...
    describe('registerStrategies', function () {
        ...
        it('registers batch job strategies as single array', () => {
            const batchJobStrategies = container.resolve(
                'batchJobStrategies'
            ) as (...args: unknown[]) => any
        ...
        it('registers batch job strategies by type and only keep the last', () => {
            ...
            expect(batchJobStrategy.constructor.name).toBe(
                'testBatch2BatchStrategy'
            )
            expect((batchJobStrategy.constructor as any).batchType).toBe(
                'type-1'
            )
    ...
    describe('registerServices', function () {
        beforeAll(() => {
            ...
            writeFileSync(
                resolve(
                    getFolderTestTargetDirectoryPath('services'),
                    'test.js'
                ),

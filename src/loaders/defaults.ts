...
export default async ({
    ...
    const currencyRepository =
        container.resolve<ReturnType<typeof CurrencyRepository>>('currencyRepository')
    const countryRepository =
        container.resolve<ReturnType<typeof CountryRepository>>('countryRepository')
    ...
    await entityManager.transaction(async (manager: EntityManager) => {
        ...
        const hasCountries = await countryRepo.find()
    ...
    await entityManager.transaction(async (manager: EntityManager) => {
        ...
        const hasCurrencies = await currencyRepo.find()
        if (hasCurrencies.length === 0) {
    ...
    await entityManager.transaction(async (manager: EntityManager) => {
        ...
        const payProviders =
            silentResolution<
                (typeof BasePaymentService | AbstractPaymentService)[]

...
class TaxProviderService extends TransactionBaseService {
    ...
    protected readonly taxLineRepo_: ReturnType<typeof LineItemTaxLineRepository>
    protected readonly smTaxLineRepo_: ReturnType<typeof ShippingMethodTaxLineRepository>
    protected readonly taxProviderRepo_: ReturnType<typeof TaxProviderRepository>
    ...
    async getShippingTaxLines(
        ...
        const smTaxLineRepo = this.manager_.withRepository(
            this.smTaxLineRepo_
        )
    ...
    async getTaxLines(
        ...
        const taxProvider = this.retrieveProvider(calculationContext.region)
        const providerLines = await taxProvider.getTaxLines(
            calculationLines.filter(
                (v) => v !== null
            ) as ItemTaxCalculationLine[],
            shippingCalculationLines,
            calculationContext
        )

        const liTaxLineRepo = this.manager_.withRepository(
            this.taxLineRepo_
        )
        const smTaxLineRepo = this.manager_.withRepository(
            this.smTaxLineRepo_

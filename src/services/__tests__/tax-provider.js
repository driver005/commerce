...
describe('TaxProviderService', () => {
    ...
    describe('getTaxLines', () => {
        ...
        test('success', async () => {
            ...
            const rates = await providerService.getTaxLines(
                cart.items,
                calcContext
            )
            ...
            expect(
                container.lineItemTaxLineRepository.create
            ).toHaveBeenCalledTimes(1)
            expect(
                container.lineItemTaxLineRepository.create
            ).toHaveBeenCalledWith(expected[0])

            expect(
                providerService.getRegionRatesForProduct
            ).toHaveBeenCalledTimes(1)
            expect(
                providerService.getRegionRatesForProduct
            ).toHaveBeenCalledWith('prod_1', region)
    ...
    describe('getRegionRatesForProduct', () => {
        ...
        test('success', async () => {
            ...
            const rates = await providerService.getRegionRatesForProduct(
                'prod_id',
                {
                    id: 'reg_id',
                    tax_rates: [],
        ...
        test('success - without product rates', async () => {
            ...
            const rates = await providerService.getRegionRatesForProduct(
                'prod_id',
                {
                    id: 'reg_id',
                    tax_rates: [
                        { id: 'reg_rate', rate: 20, name: 'PTR', code: 'ptr' },
                    ],
                    tax_rate: 12.5,
                }
            )
            ...
            expect(
                container.taxRateService.listByProduct
            ).toHaveBeenCalledTimes(1)

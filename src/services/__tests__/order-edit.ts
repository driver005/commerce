...
describe('OrderEditService', () => {
    ...
    const orderEditRepository = MockRepository({
        findOne: (query) => {
            ...
            if (
                query?.where?.id === IdMap.getId('order-edit-update-line-item')
            ) {
                return {
    ...
    const orderEditService = new OrderEditService({
        ...
        taxProviderService:
            taxProviderServiceMock as unknown as TaxProviderService,
    })
    ...
    it('should add a line item to an order edit', async () => {
        jest.spyOn(orderEditService, 'refreshAdjustments').mockImplementation(
            async () => {}
        )
        ...
        describe('cancel', () => {
            ...
            test.each(['confirmed', 'declined'])(
                ...
                async (status) => {
                    ...
                    try {
                        await orderEditService.cancel(id, {
                            loggedInUserId: userId,
                        })
                    } catch (err) {

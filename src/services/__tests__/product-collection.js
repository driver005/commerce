...
describe('ProductCollectionService', () => {
    ...
    describe('create', () => {
        const productCollectionRepository = MockRepository({
            findOne: (query) =>
                Promise.resolve({ id: IdMap.getId('bathrobe') }),
        })
    ...
    describe('update', () => {
        ...
        it('fails on non-existing product collection', async () => {
            try {
                ...
                expect(error.message).toBe(
                    `Product collection with id: ${IdMap.getId(
                        'test'
                    )} was not found`
                )
            }
    ...
    describe('delete', () => {
        ...
        it('successfully removes a product collection', async () => {
            ...
            expect(
                productCollectionRepository.softRemove
        ...
        it('succeeds idempotently', async () => {
            const result = await productCollectionService.delete(
                IdMap.getId('test')
            )

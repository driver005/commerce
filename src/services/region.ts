...
type InjectedDependencies = {
    ...
    regionRepository: ReturnType<typeof RegionRepository>
    countryRepository: ReturnType<typeof CountryRepository>
    currencyRepository: ReturnType<typeof CurrencyRepository>
    taxProviderRepository: ReturnType<typeof TaxProviderRepository>
    paymentProviderRepository: ReturnType<typeof PaymentProviderRepository>
    fulfillmentProviderRepository: ReturnType<typeof FulfillmentProviderRepository>
}
...
class RegionService extends TransactionBaseService {
    ...
    protected readonly regionRepository_: ReturnType<typeof RegionRepository>
    protected readonly countryRepository_: ReturnType<typeof CountryRepository>
    protected readonly currencyRepository_: ReturnType<typeof CurrencyRepository>
    // eslint-disable-next-line max-len
    protected readonly paymentProviderRepository_: ReturnType<typeof PaymentProviderRepository>
    // eslint-disable-next-line max-len
    protected readonly fulfillmentProviderRepository_: ReturnType<typeof FulfillmentProviderRepository>
    protected readonly taxProviderRepository_: ReturnType<typeof TaxProviderRepository>
    ...
    async create(data: CreateRegionInput): Promise<Region> {
        return await this.atomicPhase_(async (manager) => {
            const regionRepository = manager.withRepository(
    ...
    >(
        ...
        if ((regionData as UpdateRegionInput).tax_provider_id) {
            const tp = await tpRepository.findOne({
                where: {id: (regionData as UpdateRegionInput).tax_provider_id as any},
            })
            if (!tp) {
                throw new MedusaError(
                    MedusaError.Types.INVALID_DATA,
        ...
        if (regionData.payment_providers) {
            region.payment_providers = await Promise.all(
                regionData.payment_providers.map(async (pId) => {
                    const pp = await ppRepository.findOne({
                        where: { id: pId },
                    })
                    if (!pp) {
                        throw new MedusaError(
        ...
        if (regionData.fulfillment_providers) {
            region.fulfillment_providers = await Promise.all(
                regionData.fulfillment_providers.map(async (fId) => {
                    const fp = await fpRepository.findOne({
                        where: { id: fId },
                    })
                    if (!fp) {
                        throw new MedusaError(
    ...
    async list(
        ...
        const regionRepo = this.manager_.withRepository(
            this.regionRepository_
        )
    ...
    async delete(regionId: string): Promise<void> {
        return await this.atomicPhase_(async (manager) => {
            const regionRepo = manager.withRepository(
                this.regionRepository_
            )
            const countryRepo = manager.withRepository(
                this.countryRepository_
            )
    ...
    async addFulfillmentProvider(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const fpRepo = manager.withRepository(
                this.fulfillmentProviderRepository_
            )
            ...
            // Check if region already has payment provider
            if (
                region.fulfillment_providers.find(({ id }) => id === providerId)
            ) {

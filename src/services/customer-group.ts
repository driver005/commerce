...
type CustomerGroupConstructorProps = {
    ...
    customerGroupRepository: ReturnType<typeof CustomerGroupRepository>
...
class CustomerGroupService extends TransactionBaseService {
    ...
    protected readonly customerGroupRepository_: ReturnType<typeof CustomerGroupRepository>
    ...
    async update(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const cgRepo = manager.withRepository(
                this.customerGroupRepository_
            )
    ...
    async listAndCount(
        ...
        const cgRepo =
            this.manager_.withRepository(this.customerGroupRepository_)
    ...
    private async handleCreationFail(
        ...
        if (error.code === PostgresError.FOREIGN_KEY_ERROR) {
            ...
            const nonExistingCustomers = ids.filter(
                (cId) =>
                    existingCustomers.findIndex((el) => el.id === cId) === -1
            )

...
type InjectedDependencies = {
    ...
    shippingProfileRepository: ReturnType<typeof ShippingProfileRepository>
...
class ShippingProfileService extends TransactionBaseService {
    ...
    protected readonly shippingProfileRepository_: ReturnType<typeof ShippingProfileRepository>
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            if (products) {
                const productServiceTx =
                    this.productService_.withTransaction(manager)
    ...
    async fetchCartOptions(cart): Promise<ShippingOption[]> {
        return await this.atomicPhase_(async (manager) => {
            ...
            if (hasCustomShippingOptions) {
                selector.id = customShippingOptions.map(
                    (cso) => cso.shipping_option_id
                )
            }
            ...
            if (hasCustomShippingOptions) {
                const customShippingOptionsMap = new Map<
                    string,
                    CustomShippingOption
                >()

                customShippingOptions.forEach((option) => {
                    customShippingOptionsMap.set(

...
type ClaimItemServicePropa = {
    manager: EntityManager
    lineItemService: LineItemService
    eventBusService: EventBusService
    claimItemRepository: ReturnType<typeof ClaimItemRepository>
    claimTagRepository: ReturnType<typeof ClaimTagRepository>
    claimImageRepository: ReturnType<typeof ClaimImageRepository>
...
class ClaimItemService extends TransactionBaseService {
    ...
    protected readonly claimItemRepository_: ReturnType<typeof ClaimItemRepository>
    protected readonly claimTagRepository_: ReturnType<typeof ClaimTagRepository>
    protected readonly claimImageRepository_: ReturnType<typeof ClaimImageRepository>
    ...
    async create(data: CreateClaimItemInput): Promise<ClaimItem> {
        return await this.atomicPhase_(async (manager) => {
            const ciRepo = manager.withRepository(
                ...
                for (const t of tags) {
                    if (t.id) {
                        ...
                        if (existing) {
                            ...
                            item.tags.push(
                                claimTagRepo.create({ value: normalized })
                            )
                        }
                    }
                }
    ...
    async list(
        ...
        const ciRepo = this.manager_.withRepository(
            this.claimItemRepository_
        )

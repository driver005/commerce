...
type InjectedDependencies = {
    ...
    storeRepository: ReturnType<typeof StoreRepository>
...
class StoreService extends TransactionBaseService {
    ...
    protected readonly storeRepository_: ReturnType<typeof StoreRepository>
     ...
     * @return The store.
     */
    async create(): Promise<Store | null> {
    ...
    async removeCurrency(code: string): Promise<any> {
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                ...
                store.currencies = store.currencies.filter(
                    (c) => c.code !== code
                )
                return await storeRepo.save(store)
            }

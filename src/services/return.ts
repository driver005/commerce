...
type InjectedDependencies = {
    ...
    returnRepository: ReturnType<typeof ReturnRepository>
    returnItemRepository: ReturnType<typeof ReturnItemRepository>
...
class ReturnService extends TransactionBaseService {
    ...
    protected readonly returnRepository_: ReturnType<typeof ReturnRepository>
    protected readonly returnItemRepository_: ReturnType<typeof ReturnItemRepository>
    ...
    async list(
        ...
        const returnRepo = this.manager_.withRepository(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const returnReasons = await this.returnReasonService_
                ...
                .list(
                    {
                        id: [
                            ...returnLines.map((rl) => rl.reason_id as string),
                        ],
                    },
            ...
            if (
                returnReasons.some(
                    (rr) => rr.return_reason_children?.length > 0
                )
            ) {
                throw new MedusaError(
                    MedusaError.Types.INVALID_DATA,
            ...
            const rItemRepo = manager.withRepository(
                this.returnItemRepository_
            )
            ...
            if (method && method.option_id) {
                ...
                const shippingTotal =
                    ...
                    taxLines.reduce(
                        (acc, tl) =>
                            acc +
                            Math.round(shippingMethod.price * (tl.rate / 100)),
                        0
                    )
    ...
    async fulfill(returnId: string): Promise<Return | never> {
        return await this.atomicPhase_(async (manager) => {
            ...
            const returnRepo = manager.withRepository(
                this.returnRepository_
            )
    ...
    async receive(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const totalRefundableAmount =
                refundAmount || returnObj.refund_amount
            ...
            for (const i of returnObj.items) {
                ...
                const returnedQuantity =
                    (lineItem.returned_quantity || 0) + i.quantity

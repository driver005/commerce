...
type InjectedDependencies = {
    ...
    shippingMethodRepository: ReturnType<typeof ShippingMethodRepository>
    addressRepository: ReturnType<typeof AddressRepository>
    paymentSessionRepository: ReturnType<typeof PaymentSessionRepository>
    lineItemRepository: ReturnType<typeof LineItemRepository>
...
class CartService extends TransactionBaseService {
    ...
    protected readonly shippingMethodRepository_: ReturnType<typeof ShippingMethodRepository>
    protected readonly cartRepository_: ReturnType<typeof CartRepository>
    protected readonly addressRepository_: ReturnType<typeof AddressRepository>
    protected readonly paymentSessionRepository_: ReturnType<typeof PaymentSessionRepository>
    protected readonly lineItemRepository_: ReturnType<typeof LineItemRepository>
    ...
    async create(data: CartCreateProps): Promise<Cart | never> {
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                ...
                if (data.email) {
                    const customer = await this.createOrFetchUserFromEmail_(
                        data.email
                    )
    ...
    async addLineItem(
        ...
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                ...
                if (
                    this.featureFlagRouter_.isFeatureEnabled('sales_channels')
                ) {
    ...
    async updateLineItem(
        ...
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                ...
                const lineItemExists = cart.items.find(
                    (i) => i.id === lineItemId
                )
                if (!lineItemExists) {
                    throw new MedusaError(
        ...
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                ...
                if (isDefined(data.region_id)) {
                    const shippingAddress =
                        typeof data.shipping_address !== 'string'
                            ...
                            : {}
                    const countryCode =
                        (data.country_code || shippingAddress?.country_code) ??
                        null
                ...
                const billingAddress =
                    data.billing_address_id ?? data.billing_address
                if (billingAddress !== undefined) {
                    await this.updateBillingAddress_(
                        cart,
                        billingAddress,
                        addrRepo
                    )
                ...
                if (
                    ...
                    if (
                        ...
                        await this.onSalesChannelChange(
                            cart,
                            data.sales_channel_id
                        )
                ...
                if (isDefined(data.discounts)) {
                    ...
                    const hasFreeShipping = cart.discounts.some(
                        ({ rule }) =>
                            rule?.type === DiscountRuleType.FREE_SHIPPING
                    )
                ...
                if (
                    ...
                    (data.customer_id &&
                        data.customer_id !== originalCartCustomer.id)
                ) {
                    await this.eventBus_
                        .withTransaction(transactionManager)
                        .emit(
    ...
    protected async updateBillingAddress_(
        ...
        addrRepo: ReturnType<typeof AddressRepository>
    ): Promise<void> {
        let address: Address
    ...
    protected async updateShippingAddress_(
        ...
        if (
            ...
            !cart.region.countries.find(
                ({ iso_2 }) => address.country_code === iso_2
            )
        ) {
            throw new MedusaError(
                MedusaError.Types.INVALID_DATA,
            ...
            async (transactionManager: EntityManager) => {
                ...
                if (
                    ...
                    !(
                        ...
                        cart.region.payment_providers.find(
                            ({ id }) => providerId === id
                        )
                    )
                ) {
        ...
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                ...
                if (total > 0) {
                    ...
                    if (
                        region.payment_providers.length === 1 &&
                        !cart.payment_session
                    ) {
            ...
            async (transactionManager: EntityManager) => {
                ...
                if (shipping_methods?.length) {
                    const shippingOptionServiceTx =
                        this.shippingOptionService_.withTransaction(
                            transactionManager
                        )
                ...
                if (cart.items?.length) {
                    const lineItemServiceTx =
                        this.lineItemService_.withTransaction(
                            transactionManager
                        )
        ...
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                ...
                await transactionManager.remove(
                    cart.items.flatMap((i) => i.tax_lines)
                )
                await transactionManager.remove(
    ...
    async decorateTotals(
        ...
        const newTotalsServiceTx =
            this.newTotalsService_.withTransaction(manager)
        ...
        if (includeTax) {
            ...
            cartShippingMethods.forEach((method) => {
                method.tax_lines =
                    taxLinesMaps.shippingMethodsTaxLines[method.id] ?? []
            })
        }
        ...
        cart.items = (cart.items || []).map((item) => {
            const itemWithTotals = Object.assign(
                item,
                itemsTotals[item.id] ?? {}
            )
            ...
            switch (key) {
                ...
                case 'shipping_total': {
                    totals.shipping_total =
                        await this.totalsService_.getShippingTotal(cart)
                    break
                }
                case 'discount_total':
                    totals.discount_total =
                        await this.totalsService_.getDiscountTotal(cart)
                    break
                case 'tax_total':
                ...
                case 'gift_card_total': {
                    const giftCardBreakdown =
                        await this.totalsService_.getGiftCardTotal(cart)
                    totals.gift_card_total = giftCardBreakdown.total
                    totals.gift_card_tax_total = giftCardBreakdown.tax_total
                    break
                }
                case 'subtotal':
                    totals.subtotal = await this.totalsService_.getSubtotal(
                        cart
                    )
                    break
                default:

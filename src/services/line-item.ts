...
type InjectedDependencies = {
    ...
    lineItemTaxLineRepository: ReturnType<typeof LineItemTaxLineRepository>
...
class LineItemService extends TransactionBaseService {
    ...
    protected readonly itemTaxLineRepo_: ReturnType<typeof LineItemTaxLineRepository>
    ...
    async list(
        ...
        const lineItemRepo = manager.withRepository(
                    ...
                    .then((lineItems) => {
                        return lineItems.map((lineItem) =>
                            lineItemRepo.create({
                                ...
                                adjustments: lineItem.adjustments.map(
                                    (adjustment) => {
                                        return {
    ...
    async generate(
        ...
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                ...
                let unit_price =
                    Number(context.unit_price) < 0 ? 0 : context.unit_price
                ...
                if (context.cart) {
                    const adjustments = await this.lineItemAdjustmentService_
                        .withTransaction(transactionManager)
                        .generateAdjustments(context.cart, lineItem, {
                            variant,
                        })
     ...
     */
    async delete(id: string): Promise<LineItem | null> {
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                const lineItemRepository =
                    transactionManager.withRepository(
                ...
                return await lineItemRepository
                    ...
                    .then(
                        (lineItem) =>
                            lineItem && lineItemRepository.remove(lineItem)
                    )
            }
        )
    }
     ...
     * @return the result of the delete operation
     */
    async deleteWithTaxLines(id: string): Promise<LineItem | null> {
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                const lineItemRepository =
                    transactionManager.withRepository(
    ...
    public createTaxLine(args: DeepPartial<LineItemTaxLine>): LineItemTaxLine {
        const itemTaxLineRepo = this.manager_.withRepository(
            this.itemTaxLineRepo_
        )

...
type InjectedDependencies = {
    ...
    orderEditRepository: ReturnType<typeof OrderEditRepository>
...
export default class OrderEditService extends TransactionBaseService {
    ...
    protected readonly orderEditRepository_: ReturnType<typeof OrderEditRepository>
    ...
    async getTotals(orderEditId: string): Promise<{
        ...
        const shipping_total = await totalsServiceTx.getShippingTotal(
            computedOrder
        )
        const { total: gift_card_total, tax_total: gift_card_tax_total } =
            await totalsServiceTx.getGiftCardTotal(computedOrder)
        const discount_total = await totalsServiceTx.getDiscountTotal(
            computedOrder
        )
    ...
    async updateLineItem(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const isOrderEditActive =
                OrderEditService.isOrderEditActive(orderEdit)
    ...
    async deleteItemChange(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            if (
                orderEdit.confirmed_at !== null ||
                orderEdit.canceled_at !== null
            ) {
                throw new MedusaError(
                    MedusaError.Types.NOT_ALLOWED,

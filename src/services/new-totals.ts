...
export default class NewTotalsService extends TransactionBaseService {
    ...
    async getLineItemTotals(
        ...
        if (!taxRate && includeTax) {
            ...
            const itemContainsTaxLines = items.some(
    ...
    protected async getLineItemTotals_(
        ...
        if (totals.tax_lines.length > 0) {
            ...
            if (
                ...
                totals.subtotal +=
                    totals.unit_price * totals.quantity -
                    totals.original_tax_total
    ...
    protected async getLineItemTotalsLegacy(
        ...
        totals.tax_total = Math.round(
            (totals.subtotal - discount_total) * taxRate
        )
    ...
    getLineItemRefund(
        ...
        const discountAmount =
            (calculationContext.allocation_map[lineItem.id]?.discount
                ?.unit_amount || 0) * lineItem.quantity
        ...
        const lineSubtotal =
            (lineItem.unit_price - taxAmountIncludedInPrice) *
                lineItem.quantity -
            discountAmount
    ...
    async getGiftCardTotals(
        ...
        const giftAmount = giftCards.reduce(
        ...
        if (region?.gift_cards_taxable) {
            result.tax_total = Math.round(
                result.total * (region.tax_rate / 100)
            )
            return result
        }

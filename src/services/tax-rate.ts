...
type TaxRateServiceProps = {
    manager: EntityManager
    productService: ProductService
    productTypeService: ProductTypeService
    shippingOptionService: ShippingOptionService
    taxRateRepository: ReturnType<typeof TaxRateRepository>
}
...
class TaxRateService extends TransactionBaseService {
    ...
    protected readonly taxRateRepository_: ReturnType<typeof TaxRateRepository>
    ...
    async create(data: CreateTaxRateInput): Promise<TaxRate> {
        return await this.atomicPhase_(async (manager: EntityManager) => {
            const taxRateRepo = manager.withRepository(
                this.taxRateRepository_
            )
    ...
    async delete(id: string | string[]): Promise<void> {
        return await this.atomicPhase_(async (manager: EntityManager) => {
            ...
            await taxRateRepo.delete(query.where as FindOptionsWhere<TaxRate>)
        })
    }
            ...
            async (err: any) => {
                if (err.code === PostgresError.FOREIGN_KEY_ERROR) {
                    ...
                    await Promise.all([
                        ...
                        ...ids.map(async (pId) =>
                            this.productService_.retrieve(pId, {
                                select: ['id'],
                            })
                        ),
                    ])
            ...
            async (err: any) => {
                if (err.code === PostgresError.FOREIGN_KEY_ERROR) {
                    ...
                    await Promise.all([
                        this.retrieve(id, { select: ['id'] }),
                        ...ids.map(async (sId) =>
                            this.shippingOptionService_.retrieve(sId, {
                                select: ['id'],
                            })
                        ),
                    ])

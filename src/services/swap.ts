...
type InjectedProps = {
    ...
    swapRepository: ReturnType<typeof SwapRepository>
...
class SwapService extends TransactionBaseService {
    ...
    protected readonly swapRepository_: ReturnType<typeof SwapRepository>
    ...
    async processDifference(swapId: string): Promise<Swap | never> {
        return await this.atomicPhase_(async (manager) => {
            ...
            if (swap.difference_due < 0) {
                ...
                if (
                    swap.payment_status ===
                    SwapPaymentStatus.DIFFERENCE_REFUNDED
                ) {
                    return swap
                }
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            if (swap.return_order && swap.return_order.shipping_method) {
                await this.lineItemService_.withTransaction(manager).create({
                    ...
                    is_return: true,
                    tax_lines: swap.return_order.shipping_method.tax_lines.map(
                        (tl) => {
    ...
    async registerCartCompletion(swapId: string): Promise<Swap | never> {
        return await this.atomicPhase_(async (manager) => {
            ...
            if (!swap.allow_backorder) {
                ...
                for (const item of items) {
                    try {
                        ...
                        if (payment) {
                            await paymentProviderServiceTx.cancelPayment(
                                payment
                            )
                        }
            ...
            swap.payment_status =
                total === 0
                    ? SwapPaymentStatus.CONFIRMED
                    : SwapPaymentStatus.AWAITING
            ...
            for (const item of swap.additional_items) {
                ...
                if (fulfillmentItem) {
                    ...
                    if (item.quantity !== fulfilledQuantity) {
                        swap.fulfillment_status =
                            SwapFulfillmentStatus.REQUIRES_ACTION
                    }
                } else {
    ...
    async createShipment(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            for (const i of swap.additional_items) {
                ...
                if (shipped) {
                    const shippedQty =
                        (i.shipped_quantity || 0) + shipped.quantity

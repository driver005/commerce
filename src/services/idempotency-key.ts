...
type InjectedDependencies = {
    ...
    idempotencyKeyRepository: ReturnType<typeof IdempotencyKeyRepository>
}
...
class IdempotencyKeyService extends TransactionBaseService {
    ...
    protected readonly idempotencyKeyRepository_: ReturnType<typeof IdempotencyKeyRepository>
    ...
    async lock(idempotencyKey: string): Promise<IdempotencyKey | never> {
        return await this.atomicPhase_(async (manager) => {
            ...
            const isLocked =
                key.locked_at &&
                new Date(key.locked_at).getTime() >
                    Date.now() - KEY_LOCKED_TIMEOUT
    ...
    async workStage(
        ...
        return await this.atomicPhase_(async (manager) => {
            const { recovery_point, response_code, response_body } =

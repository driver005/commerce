...
type InjectedDependencies = {
    ...
    returnReasonRepository: ReturnType<typeof ReturnReasonRepository>
}

class ReturnReasonService extends TransactionBaseService {
    protected readonly retReasonRepo_: ReturnType<typeof ReturnReasonRepository>
    ...
    async create(data: CreateReturnReason): Promise<ReturnReason | never> {
        return await this.atomicPhase_(async (manager) => {
            ...
            if (
                data.parent_return_reason_id &&
                data.parent_return_reason_id !== ''
            ) {
                const parentReason = await this.retrieve(
                    data.parent_return_reason_id
                )

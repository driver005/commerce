...
type InjectedDependencies = {
    ...
    paymentCollectionRepository: ReturnType<typeof PaymentCollectionRepository>
...
export default class PaymentCollectionService extends TransactionBaseService {
    ...
    protected readonly paymentCollectionRepository_: ReturnType<typeof PaymentCollectionRepository>
    ...
    async create(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const paymentCollectionToCreate =
                paymentCollectionRepository.create({
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            for (const key of Object.keys(data)) {
                if (key === 'metadata' && data.metadata) {
                    paymentCollection[key] = setMetadata(
                        paymentCollection,
                        data.metadata
                    )
    ...
    async delete(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const paymentCollection = await this.retrieve(
                paymentCollectionId
            ).catch(() => void 0)

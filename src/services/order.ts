...
class OrderService extends TransactionBaseService {
    ...
    protected readonly orderRepository_: ReturnType<typeof OrderRepository>
    ...
    async listAndCount(
        ...
        const orderRepo = this.manager_.withRepository(
        ...
        const query = buildQuery(selector, config) as any
                ...
                qb.andWhere(
                    new Brackets((qb) => {
                        qb.where(`shipping_address.first_name ILIKE :qfn`, {
                            ...
                            .orWhere(`display_id::varchar(255) ILIKE :dId`, {
                                dId: `${q}`,
                            })
                    })
                )
    ...
    async retrieveByExternalId(
        ...
        query.relations = this.getTotalsRelations({
    ...
    async createFromCart(cartOrId: string | Cart): Promise<Order | never> {
        return await this.atomicPhase_(async (manager) => {
            ...
            const lineItemServiceTx =
                this.lineItemService_.withTransaction(manager)

            await Promise.all(
                [
                    cart.items.map((item) => {
                        return [
                            lineItemServiceTx.update(item.id, {
                                order_id: order.id,
                            }),
                    ...
                    cart.shipping_methods.map((method) => {
                        ...
                        // We normally should only pass what is needed?
                        ;(method.tax_lines as any) = undefined
                        return shippingOptionServiceTx.updateShippingMethod(
                            method.id,
                            {
                                order_id: order.id,
                            }
    ...
    async createShipment(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            for (const item of order.items) {
                const shipped = shipmentRes.items.find(
                    (si) => si.item_id === item.id
                )
                if (shipped) {
                    const shippedQty =
                        (item.shipped_quantity || 0) + shipped.quantity
    ...
    protected async updateBillingAddress(
        ...
        const addrRepo = this.manager_.withRepository(
    ...
    async cancel(orderId: string): Promise<Order> {
        return await this.atomicPhase_(async (manager) => {
            ...
            const throwErrorIf = (
                ...
                pred: (
                    obj: Fulfillment | Return | Swap | ClaimOrder
                ) => boolean,
                type: string
            ): void | never => {
            ...
            switch (totalField) {
                case 'shipping_total': {
                    order.shipping_total =
                        await this.totalsService_.getShippingTotal(order)
                    break
                }
                case 'gift_card_total': {
                    const giftCardBreakdown =
                        await this.totalsService_.getGiftCardTotal(order)
                ...
                case 'discount_total': {
                    order.discount_total =
                        await this.totalsService_.getDiscountTotal(order)
                    break
                }
                case 'tax_total': {
                    order.tax_total = await this.totalsService_.getTaxTotal(
                        order
                    )
                    break
                }
                case 'subtotal': {
                    order.subtotal = await this.totalsService_.getSubtotal(
                        order
                    )
                    break
                }
                ...
                case 'refunded_total': {
                    order.refunded_total =
                        this.totalsService_.getRefundedTotal(order)
                    break
                }
                case 'paid_total': {
                ...
                case 'items.refundable': {
                    ...
                    for (const item of order.items) {
                        items.push({
                            ...item,
                            refundable:
                                await this.totalsService_.getLineItemRefund(
                                    order,
                                    {
                                        ...item,
                                        quantity:
    ...
    async registerReturnReceived(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const refundAmount =
                customRefundAmount || receivedReturn.refund_amount

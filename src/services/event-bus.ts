...
type InjectedDependencies = {
    ...
    logger: Logger
    stagedJobRepository: ReturnType<typeof StagedJobRepository>
    redisClient: Redis
...
export default class EventBusService {
    ...
    protected readonly stagedJobRepository_: ReturnType<typeof StagedJobRepository>
    ...
    protected readonly cronQueue_: Bull.Queue
    ...
    constructor(
        ...
        const opts = {
            createClient: (type: string, redisOpts: RedisOptions): Redis => {
                switch (type) {
                    case 'client':
                        return redisClient
                    case 'subscriber':
                    ...
                    default:
                        if (config.projectConfig.redis_url) {
                            return new Redis(config.projectConfig.redis_url, { maxRetriesPerRequest: null, enableReadyCheck: false })
                        }
                        return redisClient
                }
            },
    ...
    async emit<T>(
        ...
        data: any,
        options: EmitOptions = {}
    ): Promise<StagedJob[] | StagedJob | void> {
        if (this.transactionManager_) {
            const stagedJobRepository =
                this.transactionManager_.withRepository(

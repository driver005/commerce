...
type InjectedDependencies = {
    ...
    shippingOptionRequirementRepository: ReturnType<typeof ShippingOptionRequirementRepository>
    shippingOptionRepository: ReturnType<typeof ShippingOptionRepository>
...
class ShippingOptionService extends TransactionBaseService {
    ...
    // eslint-disable-next-line max-len
    protected readonly requirementRepository_: ReturnType<typeof ShippingOptionRequirementRepository>
    protected readonly optionRepository_: ReturnType<typeof ShippingOptionRepository>
    protected readonly methodRepository_: ReturnType<typeof ShippingMethodRepository>
    ...
    async validateRequirement_(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const reqRepo = manager.withRepository(
    ...
    async retrieve(
        ...
        const manager = this.manager_
        const soRepo = manager.withRepository(
    ...
    async deleteShippingMethods(
        ...
        return await this.atomicPhase_(async (manager) => {
            const methodRepo = manager.withRepository(
    ...
    async createShippingMethod(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const validatedData =
                await this.providerService_.validateFulfillmentData(
                    option,
                    data,
                    config.cart || {}
                )
            ...
            if (typeof config.price === 'number') {
                ...
                methodPrice = await this.getPrice_(
                    option,
                    validatedData,
                    config.cart
                )
        ...
        return this.atomicPhase_(async (manager) => {
            ...
            if (isDefined(update.requirements)) {
                ...
                for (const r of update.requirements) {
                    const validated = await this.validateRequirement_(
                        r,
                        optionId
                    )
    ...
    async removeRequirement(
        requirementId
    ): Promise<DeepPartial<ShippingOptionRequirement>[] | void> {
        return await this.atomicPhase_(async (manager) => {
            ...
            return await reqRepo.softRemove(requirement as any)
        })
    }

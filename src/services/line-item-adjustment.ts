...
type LineItemAdjustmentServiceProps = {
    ...
    lineItemAdjustmentRepository: ReturnType<typeof LineItemAdjustmentRepository>
...
class LineItemAdjustmentService extends TransactionBaseService {
    ...
    private readonly lineItemAdjustmentRepo_: ReturnType<typeof LineItemAdjustmentRepository>
    ...
    async retrieve(
        ...
        const lineItemAdjustmentRepo: ReturnType<typeof LineItemAdjustmentRepository> =
    ...
    async create(
        ...
        return await this.atomicPhase_(async (manager: EntityManager) => {
            const lineItemAdjustmentRepo = manager.withRepository(this.lineItemAdjustmentRepo_)
    ...
    async update(
        ...
        return await this.atomicPhase_(async (manager: EntityManager) => {
            ...
            if (metadata) {
                lineItemAdjustment.metadata = setMetadata(
                    lineItemAdjustment,
                    metadata
                )
            }
    ...
    async delete(
        ...
        return this.atomicPhase_(async (manager) => {
            ...
            if (
                typeof selectorOrIds === 'string' ||
                Array.isArray(selectorOrIds)
            ) {
                const ids =
                    typeof selectorOrIds === 'string'
                        ? [selectorOrIds]
    ...
    async generateAdjustments(
        ...
        return this.atomicPhase_(async (manager) => {
            ...
            const amount =
                await this.discountService.calculateDiscountForLineItem(
                    discount.id,
                    lineItem,
                    cart
                )
    ...
    async createAdjustments(
        ...
        return await Promise.all(
            cart.items.map(async (li) =>
                this.createAdjustmentForLineItem(cart, li)
            )
        )
    }
}

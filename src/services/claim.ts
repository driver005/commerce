...
type InjectedDependencies = {
    ...
    claimRepository: ReturnType<typeof ClaimRepository>
...
export default class ClaimService extends TransactionBaseService {
    ...
    protected readonly claimRepository_: ReturnType<typeof ClaimRepository>
            ...
            async (transactionManager: EntityManager) => {
                ...
                if (shipping_methods) {
                    ...
                    for (const m of claim.shipping_methods) {
                        await shippingOptionServiceTx.updateShippingMethod(
                            m.id,
                            {
                                claim_order_id: null,
                            }
                ...
                if (claim_items) {
                    const claimItemServiceTx =
                        this.claimItemService_.withTransaction(
                            transactionManager
                        )
        ...
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                ...
                for (const item of claim_items) {
                    const line = await lineItemServiceTx.retrieve(
                        item.item_id,
                        {
                            relations: [
                                'order',
                ...
                if (
                    ...
                    typeof refund_amount === 'undefined'
                ) {
                ...
                if (isDefined(additional_items)) {
                    const inventoryServiceTx =
                        this.inventoryService_.withTransaction(
                            transactionManager
                        )
                ...
                const created = claimRepo.create({
                    ...
                    payment_status:
                        type === ClaimType.REFUND ? 'not_refunded' : 'na',
            ...
            async (transactionManager: EntityManager) => {
                ...
                for (const fulfillment of fulfillments) {
                    successfullyFulfilledItems =
                        successfullyFulfilledItems.concat(fulfillment.items)
                }
                    ...
                    if (fulfillmentItem) {
                        ...
                        if (item.quantity !== fulfilledQuantity) {
                            claim.fulfillment_status =
                                ClaimFulfillmentStatus.REQUIRES_ACTION
                        }
                ...
                const eventBusTx =
                    this.eventBus_.withTransaction(transactionManager)

                for (const fulfillment of fulfillments) {
                    await eventBusTx.emit(
        ...
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                ...
                for (const additionalItem of claim.additional_items) {
                    ...
                    if (shipped) {
                        const shippedQty =
                            (additionalItem.shipped_quantity || 0) +
                            shipped.quantity
                        ...
                        additionalItem.shipped_quantity !==
    ...
    async cancel(id: string): Promise<ClaimOrder> {
        return await this.atomicPhase_(
            async (transactionManager: EntityManager) => {
                ...
                if (
                    claim.return_order &&
                    claim.return_order.status !== 'canceled'
                ) {
                    throw new MedusaError(
                        MedusaError.Types.NOT_ALLOWED,
    ...
    async list(
        ...
        const claimRepo = manager.withRepository(this.claimRepository_)

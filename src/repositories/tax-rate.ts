...
export const TaxRateRepository = (manager: EntityManager) => {
    return manager.getRepository(TaxRate).extend({
        getFindQueryBuilder(findOptions: any) {
        ...
        async addToProductType(
            ...
            if (overrideExisting) {
                await this.createQueryBuilder()
                    ...
                    .where({
                        rate_id: id,
                        product_type_id: Not(In(productTypeIds)),
                    })
                    .execute()
            }

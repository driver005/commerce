...
export const MoneyAmountRepository = (manager: EntityManager) => {
    return manager.getRepository(MoneyAmount).extend({
            ...
            const pricesNotInPricesPayload = await this.createQueryBuilder()
                ...
                .andWhere(
                    new Brackets((qb) => {
                        qb.where({
                            currency_code: Not(
                                In(prices.map((p) => p.currency_code))
                            ),
                        }).orWhere({
                            region_id: Not(In(prices.map((p) => p.region_id))),
                        })
                    })
                )
                .getMany()
        ...
        async findManyForVariantInRegion(
            ...
            const qb = this.createQueryBuilder('ma')
                ...
                .andWhere(
                    "(ma.price_list_id is null or price_list.status = 'active')"
                )
                .andWhere(

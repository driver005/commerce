...
describe('TransactionBaseService', () => {
    it('should cloned the child class withTransaction', () => {
        ...
        expect(child.message()).toBe(
            `child class message method called with title title`
        )
        ...
        expect(child2.message()).toBe(
            `child class message method called with title title`
        )
        expect(child2.getTransactionManager()).toBeTruthy()
        expect((child2.getTransactionManager() as any)?.testProp).toBe(
            'testProp'
        )
    })
})

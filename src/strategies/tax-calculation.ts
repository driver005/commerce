...
class TaxCalculationStrategy implements ITaxCalculationStrategy {
    ...
    private calculateLineItemsTax(
        ...
        for (const item of items) {
            ...
            const filteredTaxLines = taxLines.filter(
                (tl) => tl.item_id === item.id
            )
            const includesTax =
            ...
            taxableAmount -=
                ((allocations.discount && allocations.discount.unit_amount) ||
                    0) * item.quantity
    ...
    private calculateShippingMethodsTax(
        ...
        for (const sm of shipping_methods) {
            const lineRates = taxLines.filter(
                (tl) => tl.shipping_method_id === sm.id
            )

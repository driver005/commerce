...
class ProductImportStrategy extends AbstractBatchJobStrategy {
    ...
    private async createProducts(
        ...
        const isSalesChannelsFeatureOn =
        ...
        for (const productOp of productOps) {
            ...
            try {
                if (
                    isSalesChannelsFeatureOn &&
                    productOp['product.sales_channels']
                ) {
                    productData['sales_channels'] =
                        await this.processSalesChannels(
                ...
                ProductImportStrategy.throwDescriptiveError(
                    productOp,
                    e.message
                )
            }
    ...
    private async createVariants(
        ...
        for (const variantOp of variantOps) {
            try {
                ...
                const optionIds =
                    (
                        variantOp['product.options'] as Record<string, string>[]
                    )?.map(
                        (variantOption) =>
                            product!.options.find(
                                (createdProductOption) =>
                                    createdProductOption.title ===
                                    variantOption.title
                            )?.id
                    ) || []

                variant.options =
                    (variant.options as Record<string, any>[])?.map(
                        (o, index) => ({
                            ...o,
            ...
            if (results[op]?.length) {
                const { writeStream, promise } = await this.fileService_
                    ...
                    .getUploadStreamDescriptor({
                        name: ProductImportStrategy.buildFilename(
                            batchJobId,
                            op
                        ),
                        ext: 'json',
        ...
        for (const op of Object.values(OperationType)) {
            try {
                await fileServiceTx.delete({
                    fileKey: ProductImportStrategy.buildFilename(
                        batchJobId,
                        op,
                        {
                            appendExt: '.json',

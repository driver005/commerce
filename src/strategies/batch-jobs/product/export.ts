...
export default class ProductExportStrategy extends AbstractBatchJobStrategy {
    ...
    async preProcessBatchJob(batchJobId: string): Promise<void> {
        return await this.atomicPhase_(async (transactionManager) => {
            ...
            const limit =
                batchJob.context?.list_config?.take ?? this.DEFAULT_LIMIT

            const { list_config = {}, filterable_fields = {} } =
                batchJob.context
            const [productList, count] = await this.productService_
                ...
                .listAndCount(filterable_fields, {
                    ...(list_config ?? {}),
                    take: Math.min(
                        batchJob.context.batch_size ?? Infinity,
                        limit
                    ),
            ...
            while (offset < productCount) {
                ...
                const shapeData =
                    this.getProductRelationsDynamicColumnsShape(products)
    ...
    async processJob(batchJobId: string): Promise<void> {
        ...
        return await this.atomicPhase_(
            async (transactionManager) => {
                ...
                offset =
                    (batchJob.context?.list_config?.skip ?? 0) +
                    advancementCount
    ...
    private appendSalesChannelsDescriptors(maxScCount: number): void {
        ...
        const columnNameDescriptionBuilder = (this
            .salesChannelsColumnsDefinition['Sales Channel Description']!
            .exportDescriptor as DynamicProductExportDescriptor)!
            .buildDynamicColumnName
    ...
    private appendOptionsDescriptors(maxOptionsCount: number): void {
        for (let i = 0; i < maxOptionsCount; ++i) {
            const columnNameNameBuilder = (this.columnsDefinition[
                'Option Name'
            ]!.exportDescriptor as DynamicProductExportDescriptor)!
                .buildDynamicColumnName
            ...
            const columnNameValueBuilder = (this.columnsDefinition[
                'Option Value'
            ]!.exportDescriptor as DynamicProductExportDescriptor)!
                .buildDynamicColumnName

    ...
    [
        'succeeds',
        {
            ...
            validate: function (value, { cartServiceMock, orderServiceMock }) {
                ...
                expect(cartServiceMock.authorizePayment).toHaveBeenCalledTimes(
                    1
                )
                ...
                expect(
                    orderServiceMock.retrieveWithTotals
    ...
    [
        'creates swap',
        {
            ...
            validate: function (value, { swapServiceMock }) {
                ...
                expect(
                    swapServiceMock.registerCartCompletion
                ).toHaveBeenCalledTimes(1)
                expect(
                    swapServiceMock.registerCartCompletion
                ).toHaveBeenCalledWith('testswap')
            },
        },
    ],
]
    ...
    describe('complete', () => {
        test.each(toTest)(
            ...
            async (title, { cart, idempotencyKey, validate }) => {
                ...
                const val = await completionStrat.complete(
                    cart.id,
                    idempotencyKey,
                    {}
                )
